...
- The `read`, `write`, and `edit` tools enforce the configured file policy in-process, then bind file I/O to an `O_NOFOLLOW` descriptor and revalidate its inode/link count before reading, truncating, or writing. This closes leaf-symlink swaps and post-start hardlink aliases while preserving symlinked parent directories, and keeps file-tool protections active even when bash is fail-closed or the OS bash sandbox is unavailable.

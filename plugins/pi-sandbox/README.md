...
- **The secret inspector scans tool *input*, not tool *output*.** The `background`/`monitor`/`jobs` integration is a second egress path: a sandboxed background command (e.g. `cat .env`) can write a secret to its job buffer, and a later `jobs action=tail` returns that buffer to the model without redaction. The default config denies *reading* `.env` (added in v0.1.0) to close the most common vector, but a sandboxed command that legitimately reads a secret-bearing file and emits it to stdout will egress via `jobs tail`. Full output inspection requires a shared redaction helper exposed by pi-sandbox and consumed by background-tasks — deferred beyond v0.1.0. Operators with strict secret-detection requirements should add the relevant paths to `filesystem.denyRead`.

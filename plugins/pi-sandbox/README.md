...
- Sandboxed bash receives a minimal child environment (`PATH`, `HOME`, `TERM`, `LANG`, `LC_*`, `TMPDIR`) and applies configured `envScrub` names and case-insensitive patterns, so registered provider or forge tokens do not pass through. Degraded background/monitor children apply the same `envScrub`. In `network.mode=block`, `TMPDIR` is always set to `/tmp` regardless of the host value.

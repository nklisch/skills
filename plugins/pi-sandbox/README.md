...
First-party Linux bubblewrap sandbox for pi. The package hardens pi's common `bash`/`user_bash` execution path with `bwrap`, enforces `read`/`write`/`edit` file policy in-process, and adds a tool-egress policy for known bypass surfaces.
...
Start pi normally after installing. The extension registers hardened replacements for `bash`, `read`, `write`, and `edit`, a `--no-sandbox` flag for intentional operator bypass, and a `/sandbox` command for diagnostics.
...
- `background` and `monitor` currently spawn outside the overridden built-in `bash` tool until the background-tasks integration lands. Track that gap in `.work/backlog/idea-background-tasks-sandbox-integration.md`.

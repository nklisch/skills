...
- **Git directory auto-discovery (submodules and linked worktrees):** when an `allowWrite` root is a git working tree whose `.git` is a *file* (gitfile) pointing outside the root — the shape used by submodules (`gitdir: ../../.git/modules/<name>`) and `git worktree add` — the sandbox automatically includes the resolved git directory in the writable surface at session start, so `git add`/`git commit` work without operator hand-configuration. A `.git` *directory* inside the working tree (the common case) needs no action. Discovery is **pinned once at session start** (trusted init state), never re-read per command — the `.git` gitfile is mutable and writable, so per-command re-reading would let an agent widen the writable surface by pointing it at another host repo. The resolved git dir must contain a `HEAD` regular file (the check git makes), so a malicious `.git` pointing at an arbitrary host path (e.g. `gitdir: /etc`) is rejected. `denyRead`/`denyWrite` always take precedence. One level of gitfile indirection only: a linked worktree's `commondir` (shared object/ref store) is not followed, so linked worktrees get per-worktree metadata access only — an operator who needs shared ref writes adds the common dir to `allowWrite` explicitly. The `background`/`monitor` tools do not auto-discover git dirs (they have no session-pinned state); commands needing git operations in a submodule should run via the sandboxed `bash` tool.
...
A separate forge-operations extension can query `Symbol.for("@nklisch/pi-sandbox.credential-boundary-capability")`; it must load file-backed credentials only when `active === true`, re-read before each load, and never treat the path- and secret-free payload as an attestation of a particular credential location. `/sandbox` reports `Credential boundary capability:` from the same contract. pi-sandbox intentionally provides no forge APIs, Git credential helper, privileged Git runner, or provider-specific remote-operation/authentication policy.
...
- The in-process file-tool policy (`read`/`write`/`edit`) checks paths then performs the filesystem operation, so a TOCTOU symlink race is possible: a concurrent process can swap a symlink between the policy check and the `read`/`write`/`edit` call. This is inherent to path-based checks (an `fd`-based `openat` design would close it) and is an accepted v0.1.0 residual; the bwrap OS layer provides the stronger boundary for sandboxed bash.

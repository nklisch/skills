...
- `session-disk` (default): a per-project dir on a disk-backed cache root (`~/.cache/pi-sandbox/tmp/<cwd-hash>/`), keyed by the canonical session cwd, bound writable with `TMPDIR` set to it. Open mode does **not** bind host `/tmp` through — sandboxed children see only their project's temp dir, not host `/tmp` accumulated files or sockets. Block mode keeps the `/tmp` tmpfs mask (IPC isolation preserved) while `TMPDIR` points at the disk dir. The dir persists across sessions (no cleanup); if disk growth ever matters, OS tooling (`systemd-tmpfiles`, a cron) handles cache-dir hygiene generically. Fail-closed if the cache root resolves to tmpfs.

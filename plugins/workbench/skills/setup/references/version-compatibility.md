...
A version gap can also change durable-prose guidance that existing foundation documents predate: writing-style rules, foundation-truth expectations, or reference conventions. When a version gap changed guidance applicable to a foundation document, audit that document against the updated reference and report conformance gaps to the user: assertions that violate a new rule, or existing assertions whose meaning conflicts with it. Present suggested rewrites for confirmation. Do not silently rewrite foundation prose as a side effect of reconciliation, and do not propose discretionary restyling of conforming prose. Repositories already conforming to the updated reference need no note.

...
| `thorough` | At least two independent passes per eligible target: correct and verify between passes until no confirmed major acceptance issue remains; nits may remain. A configurable three-pass-default limit bounds the cycle. |
...
three); work-kickoff user direction overrides the convention. At that cap,
unresolved major issues block
closure but nits do not. Review weight controls pass depth and repetition; simplification posture

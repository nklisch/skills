...
class MainTest(unittest.TestCase):
    ...
    def test_emits_additional_context_json(self) -> None:
        ...
        self.assertIn("scan, park, research, or release", context)
        ...
        self.assertIn("Overbuilding calibration", context)
        self.assertIn("Designs and reviews", context)
        self.assertIn("loose requests", context)
        self.assertIn("acquire other Workbench mechanics", context)
        self.assertIn("Pass the calibration to delegated roles", context)
        self.assertIn("must not invent requirements", context)
        self.assertIn("flag overbuilding", context)
        self.assertIn("Align optional design review once per run", context)
        self.assertIn("shared reviews across features or deliveries", context)
        self.assertIn("keep deferred review visible", context)
        self.assertIn("references/review.md", context)
        self.assertIn("constraint lens", context)
        self.assertIn("Report useful findings, evidence, and material limits", context)
        self.assertIn("continuous outcome owner", context)
        self.assertIn("Reuse unchanged context", context)
        self.assertIn("use another when it adds enough", context)
        self.assertIn("outcome owner verifies and", context)
        self.assertIn("Work items are the work record", context)
        self.assertIn("Keep foundations at durable repository", context)
        self.assertIn("engineering shape contributors need", context)

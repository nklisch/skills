...
4. **Resume the normal walk** (attest → synthesize → lint → verify) over the **current** substrate at
   ...
   superseding artifact's provenance records the **delta re-verified** + the **grandfathered**
   prior-verified set — so the verification scope stays auditable (un-recorded grandfathering is the
   §221 failure). Output is a superseding artifact with a `supersedes` pointer; the prior is retained

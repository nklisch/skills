...
| **Adopted ARD version** | [`ard.json`](../ard.json) → `adopts` | only when we re-sync to a new ARD release; it is *metadata*, never the plugin's version | **v0.5.1** |
...
- **The drift tool** — `python3 plugins/agentic-research/scripts/ard-sync.py --ard-repo <ARD-checkout>`
  ...
  **v0.4.0 → v0.4.1**, **v0.4.1 → v0.5.0**, and **v0.5.0 → v0.5.1** tool-driven — `ard-sync` reported each drift

//! Error types for the research-view-core crate.
//!
...
//!   Covers: invalid YAML, unreadable file, and any other per-artifact failure.
...
/// A non-fatal per-artifact failure. The artifact is skipped; load continues.
///
/// Covers two cases:
/// 1. The file could not be read (I/O).
/// 2. The YAML frontmatter is syntactically invalid or the file has no
...
/// Never returned for malformed artifact files (those become `ParseError`s).
#[derive(Debug)]
pub enum LoadError {
    /// An I/O error reading the substrate root directory or one of the tier
    /// directories during initial traversal.
    Io(std::io::Error),
}

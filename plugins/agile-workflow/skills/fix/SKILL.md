...
name: fix
description: >
  Diagnose and repair a specific bug as a single-stride substrate story. Reproduces
  the issue, identifies the root cause, writes a failing test, applies the minimal fix,
  confirms, and creates a story item at .work/active/stories/<id>.md at stage:review
  capturing the work. Use when something is verifiably broken — not for unverified

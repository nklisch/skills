...
and not a code refactor. The right vehicle is the discussion itself (Workbench
`ideate`): resolve Q1 and Q2 by reasoning about the tradeoffs (the attested OKF
spec, the library's stated direction, the discipline invariants, the consumer
risk surface), record the conclusions as `## Design decisions` in this epic
body, and *then* decompose into child features under `work`.

**Why not a research engagement:** the Workbench `research` skill grounds claims
about *external fetched sources* (per-source attestation, citation chains). OKF
is already attested (the first engagement did that). What's unattested here
isn't a source — it's internal artifacts (the library VISION, the SNC wiki
code, ARD's own SPEC) that the discipline machinery has nothing to bite on.
Forcing a research engagement would be empty ceremony: it'd produce a "brief"
with no real citation chain because there's nothing external to be source-bound
to. The guard against ungrounded adoption claims is the discipline applied *to
the discussion* (don't assert ungrounded claims about OKF; cite the attested
spec), not a research engagement.
...
**Why not decompose yet:** decomposition settles a *settled* epic into child
features. Running it before Q1/Q2 resolve would bake an answer to still-open
architectural questions into the children. It happens *after* the design
decisions land.
...
**Source**: `.research/briefs/okf-format-assessment-against-ard-substrate.md`

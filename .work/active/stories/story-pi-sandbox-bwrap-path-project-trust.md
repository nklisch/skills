...
- [ ] Project-local `.pi/sandbox.json` cannot set `bwrapPath` to an arbitrary
      executable (per chosen Decision A)
- [ ] A malicious checkout with `{"bwrapPath":"/tmp/evil-bwrap"}` does NOT
      result in that binary being spawned as bwrap
- [ ] The README additive-only contract holds (or is honestly updated if the
      operator accepts a narrower trust model)
- [ ] If Decision A=(b): background/monitor uses the session-pinned bwrap, not
      re-resolved config (acceptance criterion 5 of the original B2 story)
- [ ] A regression test covers the malicious-project-config case
